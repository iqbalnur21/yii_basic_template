...
class PegawaiController extends Controller
{
    ...
    {
        $model = new Pegawai();
        $personal = Personal::getAllPersonal();
        $status_pegawai = Pegawai::STATUS_PEGAWAI;
        $jenis_pegawai = Pegawai::JENIS_PEGAWAI;

        if ($this->request->isPost) {
            if ($model->load($this->request->post())) {
                $model->tanggal_bergabung = date('Y-m-d', strtotime($model->tanggal_bergabung));
                $model->save();
                Yii::$app->session->setFlash('success', 'Data Pegawai Berhasil di Tambah');
    ...
    public function actionUpdate($id_pegawai)
    {
        ...
        if ($this->request->isPost && $model->load($this->request->post())) {
            $model->tanggal_bergabung = date('Y-m-d', strtotime($model->tanggal_bergabung));
            $model->save();
            Yii::$app->session->setFlash('success', 'Data Pegawai Berhasil di Edit');
            return $this->redirect(['view', 'id_pegawai' => $model->id_pegawai]);
        }
    ...
    public function actionDelete($id_pegawai)
    {
        ...
        Yii::$app->session->setFlash('danger', 'Data Pegawai Berhasil di Hapus');
